<%-- 
    Document   : AdminTotalWorkerList
    Created on : 22-Jul-2026, 3:51:28 pm
    Author     : huzaif
--%>
<%@page import="java.util.List"%>
<%@page import="java.util.Map"%>
<%
    List<Map<String, String>> totalWorkers = (List<Map<String, String>>) request.getAttribute("totalWorkers");
...
<html>
    <head>
        ...
        <link rel="stylesheet" href="${pageContext.request.contextPath}/CSS/AdminTotalWorkerList.css">
        ...
        <div class="container">
            ...
            <div class="page-header">
                <div>
                    ...
                    <p>Manage all registered workers from here.</p>
                </div>
                <div class="worker-count">
                    <span>Total Workers</span>
                    <h2><%=session.getAttribute("workerCount")%></h2>
                </div>
            </div>
            <!-- ================= Search ================= -->
            ...
            <div class="table-container">
                <table id="workerTable">
                    ...
                    <tbody>
                        <%
                            if (totalWorkers != null && !totalWorkers.isEmpty()) {
                                for (Map<String, String> worker : totalWorkers) {
                        %>
                        <tr>
                            <td><%= worker.get("w_id")%></td>
                            <td><%= worker.get("name")%></td>
                            <td><%= worker.get("phone")%></td>
                            <td><%= worker.get("email")%></td>
                            <td>
                                <% if ("Banned".equals(worker.get("status"))) {
                                %>
                            ...
                            <td>
                                <form action="adminviewdetailsmodel" method="post">
